...
class TestLapsGet(unittest.TestCase):
    ...
    def test_laps_get(self, mock_request):
        req = make_laps_get(self.token, "test/events/test1/devices/test2/laps", success_callback=self.success_cb)
        self.assertEqual(req._method, "GET")
        self.assertTrue("test/events/test1/devices/test2/laps" in req.url)
    ...
    def test_error_callback(self, mock_request):
        error_cb = Mock()
        req = make_laps_get(self.token, "test/events/test1/devices/test2/laps", failure_callback=error_cb)
    ...
    def test_redirect_callback(self, mock_request):
        redir_cb = Mock()
        req = make_laps_get(self.token, "test/events/test1/devices/test2/laps", redirect_callback=redir_cb)
    ...
    def test_progress_callback(self, mock_request):
        progress_cb = Mock()
        req = make_laps_get(self.token, "test/events/test1/devices/test2/laps", progress_callback=progress_cb)
...
class TestFriendshipGet(unittest.TestCase):
    ...
    def test_lap_get(self, mock_request):
        req = make_lap_get(self.token, "test/events/test1/devices/test2/laps/1", success_callback=self.success_cb)
        self.assertEqual(req._method, "GET")
        self.assertEqual(req.url, "test/events/test1/devices/test2/laps/1?expand=True")
        self.assertEqual(req.req_headers["Content-Type"], "application/x-www-form-urlencoded")
        self.assertEqual(req.req_headers["Authorization"], "Bearer {}".format(self.token.token))
        self.assertEqual(req.req_headers["Accept"], "application/json")
        # simulate successful request
    ...
    def test_error_callback(self, mock_request):
        error_cb = Mock()
        req = make_lap_get(self.token, "test/events/test1/devices/test2/laps/1", failure_callback=error_cb)
        # simulate calling the requests on_error
        req.on_error()(req, {})
        # assert our lambda called the mock correctly
        error_cb.assert_called_with(
    ...
    def test_redirect_callback(self, mock_request):
        redir_cb = Mock()
        req = make_lap_get(self.token, "test/events/test1/devices/test2/laps/1", redirect_callback=redir_cb)
        # simulate calling the requests on_redirect
        req.on_redirect()(req, {})
        # assert our lambda called the mock correctly
        redir_cb.assert_called_with(
    ...
    def test_progress_callback(self, mock_request):
        progress_cb = Mock()
        req = make_lap_get(self.token, "test/events/test1/devices/test2/laps/1", progress_callback=progress_cb)
        # simulate calling the requests on_progress
        req.on_progress()(req, 0, 10)
        # assert our lambda called the mock correctly
        progress_cb.assert_called_with(
