...
class TestEventDeviceCreate(unittest.TestCase):
    def setUp(self):
        ...
        self.result_json = {
            "location": "https://podium.live/api/v1/events/" "test_event/devices/test_device",
    ...
    def test_eventdevice_create(self, mock_request):
        req = make_eventdevice_create(
            self.token, "test_event", "test_device", "test name", "1234", redirect_callback=self.success_cb
        )
        self.assertEqual(req._method, "POST")
        self.assertEqual(req.url, "https://podium.live/api/v1/events/test_event/devices")
        self.assertEqual(
            req.req_body,
            urlencode(
                {
                    "eventdevice[device_id]": "test_device",
    ...
    def test_error_callback(self, mock_request):
        error_cb = Mock()
        req = make_eventdevice_create(
            self.token, "test_event", "test_device", "test name", "1234", failure_callback=error_cb
        )
    ...
    def test_success_callback(self, mock_request):
        success_cb = Mock()
        req = make_eventdevice_create(
            self.token, "test_event", "test_device", "test name", "1234", success_callback=success_cb
        )
    ...
    def test_progress_callback(self, mock_request):
        progress_cb = Mock()
        req = make_eventdevice_create(
            self.token, "test_event", "test_device", "test name", "1234", progress_callback=progress_cb
        )
...
class TestEventDevicesGet(unittest.TestCase):
    def setUp(self):
        ...
        self.result_json = {
            "id": "test_event_id",
            "URI": "https://podium.live/api/v1/events/" "test_event/devices/test_device",
    ...
    def test_events_get(self, mock_request):
        req = make_eventdevices_get(
            self.token, event_id="test_event", per_page=100, start=0, success_callback=self.success_cb
        )
        self.assertEqual(req._method, "GET")
        self.assertTrue("https://podium.live/api/v1/events/test_event/devices" in req.url)
    ...
    def test_livestreams_get(self, mock_request):
        req = make_livestreams_get(self.token, per_page=100, start=0, success_callback=self.success_cb)
        self.assertEqual(req._method, "GET")
        self.assertTrue("https://podium.live/api/v1/livestreams" in req.url)
        self.assertTrue("per_page=100" in req.url)
        self.assertTrue("start=0" in req.url)
        self.assertTrue("expand=True" in req.url)
        self.assertEqual(req.req_headers["Content-Type"], "application/x-www-form-urlencoded")
    ...
    def test_error_callback(self, mock_request):
        error_cb = Mock()
        req = make_eventdevices_get(self.token, event_id="test_event", per_page=100, start=0, failure_callback=error_cb)
        # simulate calling the requests on_error
        req.on_error()(req, {})
        # assert our lambda called the mock correctly
        error_cb.assert_called_with(
    ...
    def test_redirect_callback(self, mock_request):
        redir_cb = Mock()
        req = make_eventdevices_get(
            self.token, event_id="test_event", per_page=100, start=0, redirect_callback=redir_cb
        )
    ...
    def test_progress_callback(self, mock_request):
        progress_cb = Mock()
        req = make_eventdevices_get(
            self.token, event_id="test_event", per_page=100, start=0, progress_callback=progress_cb
        )
        # simulate calling the requests on_progress
        req.on_progress()(req, 0, 10)
        # assert our lambda called the mock correctly
...
class TestEventDeviceDelete(unittest.TestCase):
    ...
    def check_results(self):
        self.assertEqual(self.result, "https://podium.live/api/v1/events/test_event/devices/test_device")
    ...
    def test_eventdevice_delete(self, mock_request):
        ...
        self.assertEqual(req.url, "https://podium.live/api/v1/events/test_event/devices/test_device")
        self.assertEqual(req.req_headers["Content-Type"], "application/x-www-form-urlencoded")
        self.assertEqual(req.req_headers["Authorization"], "Bearer {}".format(self.token.token))
        self.assertEqual(req.req_headers["Accept"], "application/json")
        # simulate successful request
    ...
    def test_error_callback(self, mock_request):
        error_cb = Mock()
        req = make_eventdevice_delete(
            self.token, "https://podium.live/api/v1/events/test_event/devices/test_device", failure_callback=error_cb
        )
        # simulate calling the requests on_error
        req.on_error()(req, {})
        # assert our lambda called the mock correctly
        error_cb.assert_called_with(
            ...
            {},
            {
                ...
                "progress_callback": None,
                "redirect_callback": None,
                "deleted_uri": "https://podium.live/api/v1/events/" "test_event/devices/test_device",
            },
        )
    ...
    def test_redirect_callback(self, mock_request):
        redir_cb = Mock()
        req = make_eventdevice_delete(
            self.token, "https://podium.live/api/v1/events/test_event/devices/test_device", redirect_callback=redir_cb
        )
        # simulate calling the requests on_redirect
        req.on_redirect()(req, {})
        # assert our lambda called the mock correctly
...
class TestEventDeviceGet(unittest.TestCase):
    ...
    def test_event_get(self, mock_request):
        req = make_eventdevice_get(
            self.token,
            "https://podium.live/api/v1/events/" "test_event/devices/test_device",
            success_callback=self.success_cb,
        )
        self.assertEqual(req._method, "GET")
        self.assertEqual(req.url, "https://podium.live/api/v1/events/" "test_event/devices/test_device?expand=True")
        self.assertEqual(req.req_headers["Content-Type"], "application/x-www-form-urlencoded")
        self.assertEqual(req.req_headers["Authorization"], "Bearer {}".format(self.token.token))
        self.assertEqual(req.req_headers["Accept"], "application/json")
        # simulate successful request
...
class TestEventDeviceUpdate(unittest.TestCase):
    ...
    def check_results(self):
        ...
        self.assertEqual(self.uri, "https://podium.live/api/v1/events/" "test_event/devices/test_device")
    ...
    def test_eventdevice_update(self, mock_request):
        ...
        self.assertEqual(
            ...
            urlencode(
                {
                    "eventdevice[device_id]": 123456,
                    "eventdevice[name]": "new_name",
                    "eventdevice[comp_number]": "1234abc",
                }
            ),
        )
    ...
    def test_error_callback(self, mock_request):
        ...
        error_cb.assert_called_with(
            ...
            {},
            {
                ...
                "progress_callback": None,
                "redirect_callback": None,
                "updated_uri": "https://podium.live/api/v1/events/" "test_event/devices/test_device",
            },
        )
