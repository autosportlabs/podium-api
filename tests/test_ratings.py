...
import podium_api
from podium_api.ratings import create_rating_redirect_handler, make_rating_create
...
class TestRatingCreate(unittest.TestCase):
    def setUp(self):
        ...
        self.result_json = {"location": "test/rateable_type/ratings", "object_type": "rating"}
        self.field_names = {}
    ...
    def test_rating_create(self, mock_request):
        req = make_rating_create(self.token, "rateable_type", 1234, 2, redirect_callback=self.success_cb)
        self.assertEqual(req._method, "POST")
        self.assertEqual(req.url, "https://podium.live/api/v1/rateable_type/1234/ratings")
        self.assertEqual(req.req_body, urlencode({"rating[rating]": 2}))
    ...
    def test_error_callback(self, mock_request):
        error_cb = Mock()
        req = make_rating_create(self.token, "rateable_type", 1234, 2, failure_callback=error_cb)
    ...
    def test_success_callback(self, mock_request):
        success_cb = Mock()
        req = make_rating_create(self.token, "rateable_type", 1234, 2, success_callback=success_cb)
    ...
    def test_progress_callback(self, mock_request):
        progress_cb = Mock()
        req = make_rating_create(self.token, "rateable_type", 1234, 2, progress_callback=progress_cb)
