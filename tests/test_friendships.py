...
from podium_api.friendships import (
    make_friendship_create, create_friendship_redirect_handler,
    make_friendship_delete, make_friendships_get, make_friendship_get
    )
...
class TestFriendshipCreate(unittest.TestCase):
    ...
    def test_friendship_create(self, mock_request):
        ...
        self.assertEqual(
            req.req_body,
            urlencode({'friendship[user_id]': '1'})
            )
...
class TestFriendshipDelete(unittest.TestCase):
    ...
    def test_error_callback(self, mock_request):
        ...
        req = make_friendship_delete(
            self.token, 'https://podium.live/api/v1/friendships/1',
            failure_callback=error_cb
            )
        #simulate calling the requests on_error
        req.on_error()(req, {})
...
class TestFriendshipsGet(unittest.TestCase):
    ...
    def test_friendships_get(self, mock_request):
        req = make_friendships_get(
            self.token, 
            'https://podium.live/api/v1/user/friendships',
            success_callback=self.success_cb
            )
        self.assertEqual(req._method, 'GET')
        self.assertTrue(
            'https://podium.live/api/v1/user/friendships' in req.url
            )
...
class TestFriendshipGet(unittest.TestCase):
    ...
    def test_friendship_get(self, mock_request):
        req = make_friendship_get(
            self.token,
            'https://podium.live/api/v1/friendships/test',
            success_callback=self.success_cb)
        self.assertEqual(req._method, 'GET')
        self.assertEqual(
            req.url, 'https://podium.live/api/v1/friendships/test?expand=True'
            )
        self.assertEqual(req.req_headers['Content-Type'],
                         'application/x-www-form-urlencoded')
        self.assertEqual(req.req_headers['Authorization'],
