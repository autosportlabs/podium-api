...
import podium_api
from podium_api.asyncreq import (
    get_json_header,
    make_request,
...
class TestMakeRequestDefault(unittest.TestCase):
    ...
    def test_body(self, mock_request):
        ...
        self.assertEqual(req.req_body, urlencode({"test1": "test2"}))

    @patch("podium_api.asyncreq.UrlRequest.run")
    def test_params(self, mock_request):
        req = make_request_default("test/test", params={"test1": "test2"})
        self.assertEqual(req.url, "test/test?" + urlencode({"test1": "test2"}))

    @patch("podium_api.asyncreq.UrlRequest.run")
    def test_params_with_params(self, mock_request):
        req = make_request_default("test/test?test3=test4", params={"test1": "test2"})
        self.assertEqual(req.url, "test/test?test3=test4&" + urlencode({"test1": "test2"}))
    ...
    def test_custom_data(self, mock_request):
        success_cb = Mock()
        req = make_request_default("test/test", success_callback=success_cb, data={"custom": "test"})
        # simulate calling the requests on_success
        req.on_success()(req, {})
        # assert our lambda called the mock correctly
        success_cb.assert_called_with(
...
class TestMakeRequestCustomSuccess(unittest.TestCase):
    ...
    def test_method(self, mock_request):
        success_handler = Mock()
        req = make_request_custom_success("test/test", success_handler, method="PUT")
        self.assertEqual(req._method, "PUT")

    @patch("podium_api.asyncreq.UrlRequest.run")
    def test_body(self, mock_request):
        success_handler = Mock()
        req = make_request_custom_success("test/test", success_handler, body={"test1": "test2"})
        self.assertEqual(req.req_body, urlencode({"test1": "test2"}))

    @patch("podium_api.asyncreq.UrlRequest.run")
    def test_header(self, mock_request):
        success_handler = Mock()
        req = make_request_custom_success("test/test", success_handler, header={"test1": "test2"})
        self.assertEqual(req.req_headers, {"test1": "test2"})
    ...
    def test_success_callback(self, mock_request):
        ...
        req.on_success()(req, {})
        # assert our lambda called the mock correctly
        success_handler.assert_called_with(
            req,
            {},
            {"success_callback": None, "failure_callback": None, "progress_callback": None, "redirect_callback": None},
        )
    ...
    def test_error_callback(self, mock_request):
        error_cb = Mock()
        success_handler = Mock()
        req = make_request_custom_success("test/test", success_handler, failure_callback=error_cb)
    ...
    def test_redirect_callback(self, mock_request):
        success_handler = Mock()
        redir_cb = Mock()
        req = make_request_custom_success("test/test", success_handler, redirect_callback=redir_cb)
    ...
    def test_progress_callback(self, mock_request):
        progress_cb = Mock()
        success_handler = Mock()
        req = make_request_custom_success("test/test", success_handler, progress_callback=progress_cb)
    ...
    def test_custom_data(self, mock_request):
        success_handler = Mock()
        req = make_request_custom_success("test/test", success_handler, data={"custom": "test"})
        # simulate calling the requests on_success
        req.on_success()(req, {})
        # assert our lambda called the mock correctly
        success_handler.assert_called_with(
...
class TestMakeRequest(unittest.TestCase):
    ...
    def test_make_request_body(self, mock_request):
        ...
        self.assertEqual(req.req_body, urlencode({"test": "test2"}))
