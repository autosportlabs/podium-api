...
import unittest
import podium_api
from podium_api.account import make_account_request, get_account_from_json
...
class TestAccountRequest(unittest.TestCase):
    ...
    def test_account(self, mock_request):
        req = make_account_request(self.token,
                                   success_callback=self.success_cb)
        self.assertEqual(req._method, "GET")
        self.assertEqual(req.url, "https://podium.live/api/v1/account")
