...
class TestAccountRequest(unittest.TestCase):
    ...
    def test_params(self, mock_request):
        req = make_account_get(self.token, expand=True, success_callback=self.success_cb)
        self.assertEqual(req._method, "GET")
        self.assertEqual(req.url, "https://podium.live/api/v1/account?expand=True")

    @patch("podium_api.asyncreq.UrlRequest.run")
    def test_account(self, mock_request):
        req = make_account_get(self.token, success_callback=self.success_cb)
        self.assertEqual(req._method, "GET")
        self.assertEqual(req.url, "https://podium.live/api/v1/account?expand=False")
    ...
    def test_error_callback(self, mock_request):
        error_cb = Mock()
        req = make_account_get(self.token, failure_callback=error_cb)
    ...
    def test_redirect_callback(self, mock_request):
        redir_cb = Mock()
        req = make_account_get(self.token, redirect_callback=redir_cb)
    ...
    def test_progress_callback(self, mock_request):
        progress_cb = Mock()
        req = make_account_get(self.token, progress_callback=progress_cb)
