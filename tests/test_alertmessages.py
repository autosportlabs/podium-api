...
from podium_api.alertmessages import (
    create_alertmessage_redirect_handler,
...
class TestAlertmessageCreate(unittest.TestCase):
    def setUp(self):
        podium_api.register_podium_application("test_id", "test_secret")
        self.token = PodiumToken("test_token", "test_type", 1)
        self.result_json = {"location": "test/alertmessages/1", "object_type": "alertmessage"}
        self.field_names = {}
    ...
    def test_get_redirect_from_json(self):
        self.result = get_redirect_from_json(self.result_json, "alertmessage")
        self.check_results()
    ...
    def test_alertmessage_create(self, mock_request):
        req = make_alertmessage_create(
            token=self.token, event_id=1, device_id=2, message="pit now", priority=1, redirect_callback=self.success_cb
        )
        self.assertEqual(req._method, "POST")
        self.assertEqual(req.url, "https://podium.live/api/v1/events/1/devices/2/alertmessages")
        self.assertEqual(req.req_body, urlencode({"alertmessage[message]": "pit now", "alertmessage[priority]": "1"}))
    ...
    def test_error_callback(self, mock_request):
        error_cb = Mock()
        req = make_alertmessage_create(
            token=self.token, event_id=1, device_id=2, message="pit now", priority=1, failure_callback=error_cb
        )
    ...
    def test_success_callback(self, mock_request):
        success_cb = Mock()
        req = make_alertmessage_create(
            token=self.token, event_id=1, device_id=2, message="pit now", priority=1, success_callback=success_cb
        )
    ...
    def test_progress_callback(self, mock_request):
        progress_cb = Mock()
        req = make_alertmessage_create(
            token=self.token, event_id=1, device_id=2, message="pit now", priority=1, progress_callback=progress_cb
        )
...
class TestAlertmessagesGet(unittest.TestCase):
    ...
    def test_alertmessages_get(self, mock_request):
        ...
        self.assertTrue("https://podium.live/api/v1/events/3/devices/3/alertmessages" in req.url)
...
class TestAlertmessageGet(unittest.TestCase):
    ...
    def test_alertmessage_get(self, mock_request):
        ...
        self.assertEqual(req._method, "GET")
        self.assertEqual(req.url, "http://localhost:3000/api/v1/events/3/devices/3/alertmessages/8?expand=True")
        self.assertEqual(req.req_headers["Content-Type"], "application/x-www-form-urlencoded")
        self.assertEqual(req.req_headers["Authorization"], "Bearer {}".format(self.token.token))
        self.assertEqual(req.req_headers["Accept"], "application/json")
        # simulate successful request
    ...
    def test_error_callback(self, mock_request):
        error_cb = Mock()
        req = make_alertmessage_get(
            self.token, "http://localhost:3000/api/v1/events/3/devices/3/alertmessages/8", failure_callback=error_cb
        )
        # simulate calling the requests on_error
        req.on_error()(req, {})
        # assert our lambda called the mock correctly
    ...
    def test_redirect_callback(self, mock_request):
        redir_cb = Mock()
        req = make_alertmessage_get(
            self.token, "http://localhost:3000/api/v1/events/3/devices/3/alertmessages/8", redirect_callback=redir_cb
        )
        # simulate calling the requests on_redirect
        req.on_redirect()(req, {})
        # assert our lambda called the mock correctly
    ...
    def test_progress_callback(self, mock_request):
        progress_cb = Mock()
        req = make_alertmessage_get(
            self.token, "http://localhost:3000/api/v1/events/3/devices/3/alertmessages/8", progress_callback=progress_cb
        )
        # simulate calling the requests on_progress
        req.on_progress()(req, 0, 10)
        # assert our lambda called the mock correctly
