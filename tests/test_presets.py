...
from podium_api.presets import (
    create_preset_redirect_handler,
    make_preset_create,
    make_preset_delete,
    make_preset_get,
    make_preset_update,
...
class TestPresetsGet(unittest.TestCase):
    ...
    def test_no_params(self, mock_request):
        ...
        self.assertEqual(req.url, "https://podium.live/api/v1/presets?start=20&per_page=20&type=type")
    ...
    def test_presets_get(self, mock_request):
        req = make_presets_get(self.token, "type", start=0, per_page=100, success_callback=self.success_cb)
    ...
    def test_error_callback(self, mock_request):
        error_cb = Mock()
        req = make_presets_get(self.token, "type", start=0, per_page=100, failure_callback=error_cb)
    ...
    def test_redirect_callback(self, mock_request):
        redir_cb = Mock()
        req = make_presets_get(self.token, "type", start=0, per_page=100, redirect_callback=redir_cb)
    ...
    def test_progress_callback(self, mock_request):
        progress_cb = Mock()
        req = make_presets_get(self.token, "type", start=0, per_page=100, progress_callback=progress_cb)
...
class TestPresetCreate(unittest.TestCase):
    def setUp(self):
        podium_api.register_podium_application("test_id", "test_secret")
        self.token = PodiumToken("test_token", "test_type", 1)
        self.result_json = {"location": "test/presets/test1", "object_type": "preset"}
        self.field_names = {}
    ...
    def test_preset_create(self, mock_request):
        req = make_preset_create(
            self.token, "name", "notes", "preset_data", 1234, 1, "image.jpg", "data", redirect_callback=self.success_cb
        )
    ...
    def test_error_callback(self, mock_request):
        error_cb = Mock()
        req = make_preset_create(
            self.token, "name", "notes", "preset", 1234, 1, "image.jpg", "data", failure_callback=error_cb
        )
        # simulate calling the requests on_error
        req.on_error()(req, {})
        # assert our lambda called the mock correctly
    ...
    def test_success_callback(self, mock_request):
        success_cb = Mock()
        req = make_preset_create(
            self.token, "name", "notes", "preset", 1234, 1, "image.jpg", "data", success_callback=success_cb
        )
    ...
    def test_progress_callback(self, mock_request):
        progress_cb = Mock()
        req = make_preset_create(
            self.token, "name", "notes", "preset", 1234, 1, "image.jpg", "data", progress_callback=progress_cb
        )
        # simulate calling the requests on_progress
        req.on_progress()(req, 0, 10)
        # assert our lambda called the mock correctly
...
class TestPresetDelete(unittest.TestCase):
    ...
    def test_preset_delete(self, mock_request):
        req = make_preset_delete(
            self.token, "https://podium.live/api/v1/presets/test", success_callback=self.success_cb
        )
    ...
    def test_error_callback(self, mock_request):
        error_cb = Mock()
        req = make_preset_delete(self.token, "https://podium.live/api/v1/presets/test", failure_callback=error_cb)
        # simulate calling the requests on_error
        req.on_error()(req, {})
        # assert our lambda called the mock correctly
        error_cb.assert_called_with(
    ...
    def test_redirect_callback(self, mock_request):
        redir_cb = Mock()
        req = make_preset_delete(self.token, "https://podium.live/api/v1/presets/test", redirect_callback=redir_cb)
        # simulate calling the requests on_redirect
        req.on_redirect()(req, {})
        # assert our lambda called the mock correctly
        redir_cb.assert_called_with(
    ...
    def test_progress_callback(self, mock_request):
        progress_cb = Mock()
        req = make_preset_delete(self.token, "https://podium.live/api/v1/presets/test", progress_callback=progress_cb)
        # simulate calling the requests on_progress
        req.on_progress()(req, 0, 10)
        # assert our lambda called the mock correctly
        progress_cb.assert_called_with(
...
class TestPresetGet(unittest.TestCase):
    ...
    @patch("podium_api.asyncreq.UrlRequest.run")
    def test_no_params(self, mock_request):
        req = make_preset_get(self.token, "https://podium.live/api/v1/presets/test", expand=None)
        self.assertEqual(req.url, "https://podium.live/api/v1/presets/test")
    ...
    def test_preset_get(self, mock_request):
        req = make_preset_get(self.token, "https://podium.live/api/v1/presets/test", success_callback=self.success_cb)
        self.assertEqual(req._method, "GET")
        self.assertEqual(req.url, "https://podium.live/api/v1/presets/test?expand=True")
        self.assertEqual(req.req_headers["Content-Type"], "application/x-www-form-urlencoded")
        self.assertEqual(req.req_headers["Authorization"], "Bearer {}".format(self.token.token))
        self.assertEqual(req.req_headers["Accept"], "application/json")
        # simulate successful request
    ...
    def test_error_callback(self, mock_request):
        error_cb = Mock()
        req = make_preset_get(self.token, "https://podium.live/api/v1/presets/test", failure_callback=error_cb)
        # simulate calling the requests on_error
        req.on_error()(req, {})
        # assert our lambda called the mock correctly
        error_cb.assert_called_with(
    ...
    def test_redirect_callback(self, mock_request):
        redir_cb = Mock()
        req = make_preset_get(self.token, "https://podium.live/api/v1/presets/test", redirect_callback=redir_cb)
        # simulate calling the requests on_redirect
        req.on_redirect()(req, {})
        # assert our lambda called the mock correctly
        redir_cb.assert_called_with(
    ...
    def test_progress_callback(self, mock_request):
        progress_cb = Mock()
        req = make_preset_get(self.token, "https://podium.live/api/v1/presets/test", progress_callback=progress_cb)
        # simulate calling the requests on_progress
        req.on_progress()(req, 0, 10)
        # assert our lambda called the mock correctly
        progress_cb.assert_called_with(
...
class TestPresetUpdate(unittest.TestCase):
    ...
    @patch("podium_api.asyncreq.UrlRequest.run")
    def test_no_params(self, mock_request):
        req = make_preset_update(self.token, "https://podium.live/api/v1/presets/test")
