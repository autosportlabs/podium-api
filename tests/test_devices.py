...
from podium_api.devices import (
    make_device_create, make_device_get, make_device_delete,
    make_device_update, create_device_redirect_handler
)
...
class TestDeviceCreate(unittest.TestCase):
    ...
    def test_device_create(self, mock_request):
        req = make_device_create(self.token, 'test', 
...
class TestDeviceDelete(unittest.TestCase):
    ...
    def check_results(self):
        self.assertEqual(self.result,
                         'https://podium.live/api/v1/devices/test')
    ...
    def test_device_delete(self, mock_request):
        req = make_device_delete(self.token,
                                 'https://podium.live/api/v1/devices/test',
...
class TestDeviceGet(unittest.TestCase):
    ...
    def test_device_get(self, mock_request):
        req = make_device_get(self.token,
                              'https://podium.live/api/v1/devices/test',
                              success_callback=self.success_cb)
        self.assertEqual(req._method, 'GET')
        self.assertEqual(req.url, 
                         'https://podium.live/api/v1/devices/test?expand=True')
        self.assertEqual(req.req_headers['Content-Type'],
                         'application/x-www-form-urlencoded')
        self.assertEqual(req.req_headers['Authorization'],
                         'Bearer {}'.format(self.token.token))
...
class TestDeviceUpdate(unittest.TestCase):
    ...
    def test_no_params(self, mock_request):
        req = make_device_update(self.token,
                                 'https://podium.live/api/v1/devices/test')
    ...
    def test_device_update(self, mock_request):
        ...
        self.assertEqual(req.req_body,
                         urlencode({'device[name]': 'new_title'}))
        self.assertEqual(req.url, 
                         'https://podium.live/api/v1/devices/test')
        self.assertEqual(req.req_headers['Content-Type'],
                         'application/x-www-form-urlencoded')
