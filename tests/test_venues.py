...
class TestVenuesGet(unittest.TestCase):
    ...
    def test_venues_get(self, mock_request):
        req = make_venues_get(self.token, "test/venues", success_callback=self.success_cb)
        self.assertEqual(req._method, "GET")
        self.assertTrue("test/venues" in req.url)
    ...
    def test_error_callback(self, mock_request):
        error_cb = Mock()
        req = make_venues_get(self.token, "test/venues", failure_callback=error_cb)
    ...
    def test_redirect_callback(self, mock_request):
        redir_cb = Mock()
        req = make_venues_get(self.token, "test/venues", redirect_callback=redir_cb)
    ...
    def test_progress_callback(self, mock_request):
        progress_cb = Mock()
        req = make_venues_get(self.token, "test/venues", progress_callback=progress_cb)
...
class TestVenueGet(unittest.TestCase):
    ...
    def test_venue_get(self, mock_request):
        req = make_venue_get(self.token, "test/venues/1234", success_callback=self.success_cb)
        self.assertEqual(req._method, "GET")
        self.assertEqual(req.url, "test/venues/1234?expand=False")
        self.assertEqual(req.req_headers["Content-Type"], "application/x-www-form-urlencoded")
        self.assertEqual(req.req_headers["Authorization"], "Bearer {}".format(self.token.token))
        self.assertEqual(req.req_headers["Accept"], "application/json")
        # simulate successful request
    ...
    def test_error_callback(self, mock_request):
        error_cb = Mock()
        req = make_venue_get(self.token, "test/venues/1234", failure_callback=error_cb)
        # simulate calling the requests on_error
        req.on_error()(req, {})
        # assert our lambda called the mock correctly
        error_cb.assert_called_with(
    ...
    def test_redirect_callback(self, mock_request):
        redir_cb = Mock()
        req = make_venue_get(self.token, "test/venues/1234", redirect_callback=redir_cb)
        # simulate calling the requests on_redirect
        req.on_redirect()(req, {})
        # assert our lambda called the mock correctly
        redir_cb.assert_called_with(
    ...
    def test_progress_callback(self, mock_request):
        progress_cb = Mock()
        req = make_venue_get(self.token, "test/venues/1234", progress_callback=progress_cb)
        # simulate calling the requests on_progress
        req.on_progress()(req, 0, 10)
        # assert our lambda called the mock correctly
        progress_cb.assert_called_with(
