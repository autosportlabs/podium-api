...
def make_alertmessage_get(token, endpoint,
                        expand=True,
    ...
    return make_request_custom_success(endpoint, alertmessage_success_handler,
                                       method="GET",
...
def make_alertmessages_get(token, event_id=None, device_id=None, endpoint=None,
    ...
    header = get_json_header_token(token)
    return make_request_custom_success(endpoint, alertmessages_success_handler,
                                       method="GET",
                                       success_callback=success_callback,
                                       failure_callback=failure_callback,
                                       progress_callback=progress_callback,
...
def make_alertmessage_create(token, event_id, device_id, message, priority,
    ...
    endpoint = '{}/api/v1/events/{}/devices/{}/alertmessages'.format(podium_api.PODIUM_APP.podium_url, event_id, device_id)
...
def alertmessage_success_handler(req, results, data):
    ...
    if data['success_callback'] is not None:
        data['success_callback'](
            get_alertmessage_from_json(results['alertmessage'])
            )
